        ...
        <>
          ...
          {confirmGlobal ? (
            ...
            <button
              ...
              disabled={!snapshot?.dockerAvailable || (summary?.activeSandboxes ?? 0) === 0}
            >
              <FaTrash />
    ...
    >
      <div className='sandboxes-page'>
        ...
        <div className='sb-infobar'>
          <div className='sb-infobar__stat'>
            <strong>{summary?.activeSandboxes ?? 0}</strong>
            <span>Sandboxes</span>
          </div>
          <div className='sb-infobar__divider' />
          <div className={`sb-infobar__stat${(summary?.zombieSandboxes ?? 0) > 0 ? ' sb-infobar__stat--warn' : ''}`}>
            <strong>{summary?.zombieSandboxes ?? 0}</strong>
            <span>Zombies</span>
          </div>
          <div className='sb-infobar__divider' />
          <div className='sb-infobar__stat'>
            <strong>{(summary?.containers ?? 0) + (summary?.networks ?? 0) + (summary?.volumes ?? 0)}</strong>
            <span>Resources</span>
          </div>
          <div className='sb-infobar__divider' />
          <div className='sb-infobar__stat'>
          ...
          <span className='sb-infobar__updated'>
            {snapshot?.updatedAt ? `Updated ${new Date(snapshot.updatedAt).toLocaleTimeString()}` : 'Ready'}
          </span>
        </div>
        ...
        {snapshot && snapshot.sandboxes.length > 0 ? (
          <div className='sb-grid'>
            ...
            {visibleSandboxes.length === 0 && (
              <div className='sb-empty-filter'>No sandboxes match the current filter.</div>
            )}
          </div>
        ) : (
          <div className='sb-empty'>
            ...
            <p>As soon as the runtime engine labels containers, networks, and volumes with <code>babelsuite.managed=true</code> and <code>babelsuite.run_id</code>, this page will become a live control surface.</p>
            <div className='sb-labels'>
              <span>babelsuite.managed=true</span>
              <span>babelsuite.run_id=run-2049</span>
              <span>babelsuite.suite=payment-suite</span>
              <span>babelsuite.profile=staging.yaml</span>
            </div>
          </div>
        )}
      </div>
            ...
            <div className='sb-panel-header__actions'>
              {confirmNuke === selectedSandbox.sandboxId ? (
                <div className='sb-confirm'>
                  ...
                  <button
                    className='sb-btn sb-btn--danger'
                    onClick={() => void runAction(() => reapSandbox(selectedSandbox.sandboxId), selectedSandbox.sandboxId, 'Sandbox reaped.')}
