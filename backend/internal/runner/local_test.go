...
func TestLocalRunnerEmitsLoadSpecificLogs(t *testing.T) {
	r := NewLocal()
	lines := make([]logstream.Line, 0)
	...
	if len(lines) < 5 {
		t.Fatalf("expected richer load logs, got %d lines", len(lines))
	}
	if lines[1].Text == "" {
		t.Fatal("expected non-empty load boot message")
	}
	...
	if !foundThresholdLog {
		t.Fatal("expected load logs to mention thresholds or ramp behavior")
	}
}
