...
func renderModuleReadme(module catalog.Package) string {
	...
	if moduleDirectoryName(module) == "runtime" {
		usageLine = "See `usage.star`, `suite_example.star`, `scripts/container_lifecycle.star`, `scripts/mock_lifecycle.star`, `scripts/service_lifecycle.star`, `scripts/script_results.star`, `load_reports.star`, and `scenario_reports.star` for the recommended runtime patterns."
		structure = []string{
			"",
			"Runtime Examples",
			"",
			"- `usage.star`: quick overview of `container.*`, native `mock.*`, external `service.*`, `script.*`, `load.*`, and `scenario.*` entry points plus object methods.",
			...
			"- `load_reports.star`: staged load execution with normalized throughput, latency, and threshold reporting.",
			...
			"- `load/`: native plan builders plus compatibility assets for code-first and imported load runners such as locust, k6, and jmx.",
...
func renderModuleExtraFiles(module catalog.Package) []RenderedFile {
	...
	return []RenderedFile{
		...
		{
			Path: "load/graphql.jmx",
			Content: strings.Join([]string{
				`<?xml version="1.0" encoding="UTF-8"?>`,
				`<jmeterTestPlan version="1.2" properties="5.0">`,
				`  <hashTree>`,
				`    <TestPlan guiclass="TestPlanGui" testclass="TestPlan" testname="GraphQL Load Plan" enabled="true">`,
				`      <stringProp name="TestPlan.comments">Sample GraphQL load plan for the runtime module.</stringProp>`,
				...
				`    </TestPlan>`,
				`    <hashTree>`,
				`      <ThreadGroup guiclass="ThreadGroupGui" testclass="ThreadGroup" testname="GraphQL Users" enabled="true">`,
				...
				`      </ThreadGroup>`,
				`      <hashTree>`,
				`        <HTTPSamplerProxy guiclass="GraphQLHTTPSamplerGui" testclass="HTTPSamplerProxy" testname="Favorite Droid" enabled="true">`,
				`          <stringProp name="HTTPSampler.path">/graphql</stringProp>`,
				`          <stringProp name="HTTPSampler.method">POST</stringProp>`,
				`          <stringProp name="GraphQLHTTPSampler.query">query { droid(id: "2001") { id name } }</stringProp>`,
				`        </HTTPSamplerProxy>`,
				`        <hashTree/>`,
				`      </hashTree>`,
				`    </hashTree>`,
