...
func (s *Store) CreateRun(ctx context.Context, r *domain.Run) error {
	_, err := s.pool.Exec(ctx,
		`INSERT INTO runs(run_id,org_id,package_id,image_ref,agent_id,status,started_at,finished_at,created_at)
		 VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9)`,
		r.RunID, r.OrgID, r.PackageID, r.ImageRef, r.AgentID, r.Status,
...
func (s *Store) ListRuns(ctx context.Context, orgID string, page, pageSize int) ([]*domain.Run, int64, error) {
	...
	rows, err := s.pool.Query(ctx,
		`SELECT run_id,org_id,package_id,image_ref,agent_id,status,started_at,finished_at,created_at
	...
	for rows.Next() {
		var r domain.Run
		if err := rows.Scan(&r.RunID, &r.OrgID, &r.PackageID, &r.ImageRef, &r.AgentID,
...
func (s *Store) GetRun(ctx context.Context, id string) (*domain.Run, error) {
	...
	err := s.pool.QueryRow(ctx,
		...
		Scan(&r.RunID, &r.OrgID, &r.PackageID, &r.ImageRef, &r.AgentID,
...
func (s *Store) NextPendingRun(ctx context.Context, orgID string) (*domain.Run, error) {
	...
	err := s.pool.QueryRow(ctx,
		`UPDATE runs SET status='running', started_at=$2
		 ...
		 RETURNING run_id,org_id,package_id,image_ref,agent_id,status,started_at,finished_at,created_at`,
		orgID, now).
		Scan(&r.RunID, &r.OrgID, &r.PackageID, &r.ImageRef, &r.AgentID,
			&r.Status, &r.StartedAt, &r.FinishedAt, &r.CreatedAt)
	return &r, wrap(err)
...
func (s *Store) AppendLogs(ctx context.Context, entries []*domain.LogEntry) error {
	...
	for _, e := range entries {
		batch.Queue(
			`INSERT INTO run_logs(log_id,run_id,step_id,line,data,time,type) VALUES($1,$2,$3,$4,$5,$6,$7) ON CONFLICT DO NOTHING`,
			e.LogID, e.RunID, e.StepID, e.Line, e.Data, e.Time, int(e.Type))
	}
...
func (s *Store) GetLogs(ctx context.Context, stepID string) ([]*domain.LogEntry, error) {
	rows, err := s.pool.Query(ctx,
		`SELECT log_id,run_id,step_id,line,data,time,type FROM run_logs WHERE step_id=$1 ORDER BY line ASC`,
		stepID)
	if err != nil {
		return nil, wrap(err)
	}
	...
	for rows.Next() {
		...
		var t int
		if err := rows.Scan(&e.LogID, &e.RunID, &e.StepID, &e.Line, &e.Data, &e.Time, &t); err != nil {
			return nil, err
		}
